...
def test_chat(provider_name: str, model_name: str, message: str):
    ...
    url = f"{BASE_URL}/chat"
    payload = {"provider_name": provider_name, "model": model_name, "message": message}
    try:
        ...
        if "response" in result and ai_response is not None:
            print(
                f"--- /chat テスト成功 (Provider: {provider_name}, Model: {model_name}) ---"
            )
            return ai_response
        else:
            print(
        ...
        print(
            f"!!! /chat テスト失敗 (Provider: {provider_name}, Model: {model_name}, リクエストエラー): {e}"
        )

        return None
    except Exception as e:
        print(
            f"!!! /chat テスト失敗 (Provider: {provider_name}, Model: {model_name}, 予期せぬエラー): {e}"
        )
        return None
...
def test_chat_no_provider():
    """/chat エンドポイントでProvider名を指定しない場合をテストする"""
    print("\n--- テスト開始: /chat エンドポイント (Provider名なし) ---")
    url = f"{BASE_URL}/chat"
    payload = {
        "model": "gpt-4o-mini",
        "message": "Provider名なしでリクエストを送信します。",
    }
    try:
        response = requests.post(url, json=payload)
        print(f"ステータスコード: {response.status_code}")
        ...
        print(f"AIからの応答: {ai_response}")
        if "response" in result and ai_response is not None:
            print("--- /chat Provider名なしテスト 成功 ---")
            return ai_response
        else:
            print(
                f"!!! /chat Provider名なしテスト 失敗: レスポンスに 'response' キーがないか、nullです。"
            )
            print(f"受信したJSON: {result}")
            return None
    except requests.exceptions.RequestException as e:
        print(f"!!! /chat Provider名なしテスト 失敗 (リクエストエラー): {e}")
        return None
    except Exception as e:
        print(f"!!! /chat Provider名なしテスト 失敗 (予期せぬエラー): {e}")
        return None
...
if __name__ == "__main__":
    ...
    if available_providers:
        provider_to_test = "Blackbox"
        model_to_test = "gpt4-o"

        if provider_to_test not in available_providers:
            print(
                f"警告: 指定されたProvider '{provider_to_test}' は /models のリストに含まれていません。リストの最初のProvider '{available_providers[0]}' で試します。"
            )
            if available_providers:
